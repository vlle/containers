...
TEST_F(MapTest, AtValueThrow) { EXPECT_ANY_THROW(my_container.at(18)); }
...
TEST_F(MapTest, IteratorMap) {
  ...
  for (auto std_map_p = expected_container.begin();
       std_map_p != expected_container.end(); ++std_map_p) {
    ASSERT_NO_THROW(my_container.at(std_map_p->first));
  }
  for (auto std_map_p = expected_container.begin();
       map_p != my_container.end() && std_map_p != expected_container.end();
       ++map_p, ++std_map_p)
